...
+(void)ramChanged {
    ...
    /* Stats in bytes */
        ...
        natural_t wired = vm_stat.wire_count * pagesize / (1024 * 1024);
        natural_t active = vm_stat.active_count * pagesize / (1024 * 1024);
        natural_t inactive = vm_stat.inactive_count * pagesize / (1024 * 1024);
        natural_t ram_free = vm_stat.free_count * pagesize / (1024 * 1024) + inactive;
        natural_t ram_used = active + wired; // Inactive is treated as free by iOS
        
        NSString *used = [NSString stringWithFormat:@"%.2u", ram_used];
        NSString *free = [NSString stringWithFormat:@"%.2u", ram_free];
        NSString *total_usable = [NSString stringWithFormat:@"%.2u", ram_total_usable];
        NSString *total_physical = [NSString stringWithFormat:@"%.2u", ram_total_physical];
